...
This skill is for read-only analysis through OneQuery-managed access plus CLI
discovery.
...
- Use it when the user wants company or customer data that is expected to be
  ...
  needs to inspect org or source context before running a narrower command.
...
- Use `onequery --help` and `onequery <command> --help` for command discovery
  before guessing flags or subcommands.
...
3. If the task is only command or skill discovery, inspect `onequery --help`
   or `onequery <command> --help` before doing anything auth-related.
